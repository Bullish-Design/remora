...
- **One layer, one dependency**: discovery, graph, execution, context, and checkpoint code live in `src/remora/`; each package has a clear dependency (Cairn for workspaces, structured-agents for kernels, Grail for scripts, fsdantic for indexes).
- **Explicit config surface**: `remora.yaml` holds project-wide settings while each agent bundle stays in structured-agents format; Remora metadata such as node types and priority hints sit in its own mappings.
- **Unified observability**: the tabulated event taxonomy is exposed via an EventBus that implements `structured_agents.events.observer.Observer`, re-emits kernel events, and surfaces Remora graph/human-IO events for services and context builders.

## Foundation
### Flattened Configuration
- `remora.yaml` is the single immutable source for discovery paths, workspace cleanup, indexer storage, dashboard binding, execution concurrency, and structured-agents client details (`model_base_url`, `api_key`, defaults).
- `bundle.yaml` files stay in the public structured-agents shape (model, grammar, limits, tools) while Remora-specific metadata (node types handled, execution priority, context requirements) live in a lightweight `BundleMetadata` record keyed by bundle name within Remora's plan.
...
### Unified EventBus
- Build an `EventBus` that `implements structured_agents.events.observer.Observer` by exposing `async def emit(self, event: Event)` so kernels, context builders, and dashboards share a single multicast.
- Re-export kernel events (`KernelStartEvent`, `RestartEvent`, `ToolCallEvent`, etc.) and augment with Remora graph/human-in-the-loop events such as `GraphStartEvent`, `AgentCompleteEvent`, `HumanInputRequestEvent`, and `HumanInputResponseEvent`.
...
- `CairnDataProvider` populates Grail files from the Cairn workspace.
- `CairnResultHandler` persists tool outputs back into Cairn layers.
...
- `GraphExecutor` delegates to `structured_agents.agent.Agent.from_bundle()` but wraps it to inject Remora context: set `STRUCTURED_AGENTS_BASE_URL` and `STRUCTURED_AGENTS_API_KEY` from `RemoraConfig.model` before loading, and provide the EventBus as the agent observer.
- Because the manifest loader ignores Remora-specific metadata, capture node type → bundle mapping plus node priority/`requires_context` data in our own `BundleMetadata` table and keep it separate from `bundle.yaml` files.
- Replace `structured_agents.tools.grail.discover_tools()` with a Remora-aware loader that instantiates each Grail script once per bundle with the correct `files` dict (from `CairnDataProvider`) and `externals` (human-in-the-loop `ask_user` wiring) and exposes a `GrailTool` subclass that stores the data provider for later result handling.
...
- Tools stay pure: inputs arrive via Grail VFS, outputs describe mutations, and `CairnResultHandler` persists changes.
...
- Checkpoints capture progress so retries resume without rerunning completed nodes.
...
- Add integration smoke tests that run a single bundle against sample code to ensure DataProvider/Cairn wiring correctly populates the virtual filesystem and that human-IO events resolve.
