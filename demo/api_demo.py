...
async def listen_to_events(stop_event: asyncio.Event, expected_completions: int):
    ...
    try:
        async with httpx.AsyncClient(timeout=httpx.Timeout(30.0, connect=10.0)) as client:
            print(f"🔌 Connecting to Event Stream at {HUB_URL}/events...")
                ...
                async for sse in event_source.aiter_sse():
                    ...
                    if sse.event == "agent:completed" or sse.event == "agent:failed":
                        ...
                        if completed_graphs >= expected_completions:
                            print("\n🛑 Received all expected agent completions. Terminating monitor.")
                            stop_event.set()
                            break
    except httpx.ReadTimeout:
        print("⏱️ Event stream timed out waiting for events")
...
async def trigger_execution(agent_bundle: str, file_path: str, target_code: str):
    ...
    print(f"\n🚀 Fire-and-forget: POST for '{agent_bundle}' bundle...")
    try:
        async with httpx.AsyncClient(timeout=httpx.Timeout(5.0, connect=3.0)) as client:
            ...
            if response.status_code == 200:
                ...
                print(f"✅ Started! Graph ID: {graph_id}")
                return graph_id
            else:
...
async def poll_for_completion(graph_ids: list[str], timeout: float = 30.0):
    ...
    print(f"\n📋 Polling for completion of graphs: {graph_ids}")
...
async def main():
    ...
    print(f"\n⏳ Started {len(graph_ids)} graphs. Listening for events...\n")
