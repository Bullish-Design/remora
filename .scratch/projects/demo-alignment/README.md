...
- `GUIDE_WEB_UI.md` — Graph viewer DB schema fixes (critical runtime SQL bugs)
...
- `GUIDE_COMPANION.md` — Event system separation rationale, LSP pattern alignment
...
1. **`panel.lua` — Live `AgentMessageEvent` silently dropped** (`src/remora/lsp/nvim/lua/remora/panel.lua`)
...
2. **`state.py` — SQL queries use stale column names** (`remora_demo/web/graph/state.py`)
   - `event_id` → should be `id` (events PK is INTEGER AUTOINCREMENT)
   - `agent_id` → should be `from_agent` / `to_agent`
   - `nodes.id` → should be `nodes.node_id`
   - Causes `sqlite3.OperationalError` at runtime
   - `tests/test_bridge.py` uses the same stale schema → tests pass but don't catch production mismatch

### Medium Priority

3. **`panel.lua` — `AgentMessageEvent` direction always "unknown"**
   - `ev.payload.from_agent`/`ev.payload.to_agent` are nil; correct fields are `ev.from_agent`/`ev.to_agent`

4. **`chat_service.py` — Module-level singleton anti-pattern** (`src/remora/service/chat_service.py`)
   - Module import creates live `ChatServiceState` and `app` objects
   - `create_app()` uses `globals()` fallback — DI is incomplete
   - Handler closures capture module-level state, not injected state

### Low Priority

5. **`panel.lua` — Duplicate user message after panel refresh** (local event id=nil not deduplicated)
...
- `remora_demo/companion/` event system separation is intentional (different semantics) ✅
