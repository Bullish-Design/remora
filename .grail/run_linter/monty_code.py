...
try:
    ...
    if isinstance(issues_data, dict) and 'error' in issues_data:
        lint_error = issues_data.get('stderr', '').strip() or str(issues_data.get('error')) or f"ruff exit code {issues_data.get('exit_code')}"
    if not lint_error and exit_code not in {0, 1}:
        lint_error = f'ruff exit code {exit_code}'
    if lint_error:
        result = {'result': None, 'summary': f'Error: {lint_error}', 'knowledge_delta': {'lint_target': file_path, 'lint_check_only': check_only}, 'outcome': 'error', 'error': lint_error}
    else:
        ...
        if total == 0:
            ...
            outcome = 'partial'
        result = {'result': raw_result, 'summary': summary, 'knowledge_delta': {'lint_errors_remaining': total, 'lint_errors_fixable': fixable_count, 'lint_target': file_path, 'lint_check_only': check_only}, 'outcome': outcome}
