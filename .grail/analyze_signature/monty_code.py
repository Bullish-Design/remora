...
def _parse_signature(line: str, is_async: bool) -> dict[str, Any]:
    ...
    return {'function_name': func_name, 'parameters': _parse_parameters(params_part), 'return_type': return_type if return_type else None, 'is_async': is_async}
try:
    if not source_code:
        raise ValueError('Source code is required.')
    signature_line, is_async = _find_signature_line(source_code)
    if not signature_line:
        ...
        result = {'result': raw_result, 'summary': summary, 'knowledge_delta': {'function_name': raw_result.get('function_name'), 'parameter_count': param_count, 'return_type': raw_result.get('return_type'), 'is_async': raw_result.get('is_async')}, 'outcome': 'success'}
