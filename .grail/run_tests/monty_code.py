...
def _parse_totals(xml: str) -> dict[str, int]:
    ...
    while True:
        ...
        index = end + 1
    return {'tests': tests_total, 'failures': failures_total, 'errors': errors_total, 'skipped': skipped_total}
...
try:
    ...
    if structured_report:
        totals = {'tests': int(structured_report.get('tests', 0)), 'failures': int(structured_report.get('failures', 0)), 'errors': int(structured_report.get('errors', 0)), 'skipped': int(structured_report.get('skipped', 0))}
        failures = [dict(item) for item in structured_report.get('failures', []) if isinstance(item, dict)]
    else:
    ...
    if failed_total == 0:
        ...
        outcome = 'partial'
    result = {'result': raw_result, 'summary': summary, 'knowledge_delta': {'tests_passed': raw_result['passed'], 'tests_failed': raw_result['failed'], 'tests_errors': raw_result['errors'], 'test_file': test_file, 'target_file': target_file}, 'outcome': outcome}
