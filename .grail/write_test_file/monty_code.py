# Auto-generated by grail — this is what Monty actually executes

try:
    if not path_value:
        raise ValueError('Test file path is required.')
    if not content:
        raise ValueError('Test content is required.')
    raw_result = {'success': True, 'path': path_value}
    result = {'result': raw_result, 'summary': f'Wrote test file {path_value}', 'knowledge_delta': {'test_file_written': path_value}, 'outcome': 'success', 'written_file': path_value, 'content': content}
except Exception as exc:
    result = {'result': {'success': False, 'error': str(exc)}, 'summary': f'Error: {exc}', 'knowledge_delta': {}, 'outcome': 'error', 'error': str(exc)}
result